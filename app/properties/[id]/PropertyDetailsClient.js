...
export default function PropertyDetailsClient({ initialProperty }) {
  ...
  const formattedProperty = {
    ...initialProperty,
    price: typeof initialProperty.price === 'number' 
      ? formatCurrency(initialProperty.price) 
      : (initialProperty.price || 'N/A')
  };

  const [property, setProperty] = useState(formattedProperty);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        switch (res.status) {
          ...
          case 400:
            ...
            setError(data.error || "Cannot delete property. Please check its status.");
            return;
          default:
      ...
      setError(error.message || "An unexpected error occurred while deleting the property.");
    } finally {
